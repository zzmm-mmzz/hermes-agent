...
async def get_hub_skills():
    """Browse all skills from the self-hosted SkillHub."""
    from tools.skills_hub import SelfHostedSkillHubSource
    try:
        source = SelfHostedSkillHubSource()
        catalog = source._load_catalog_index_urllib()
        return [
            {
                "name": s.name,
                "description": s.description,
                "category": s.tags[0] if isinstance(s.tags, list) and s.tags else "",
                "enabled": False,
                "source": s.source,
