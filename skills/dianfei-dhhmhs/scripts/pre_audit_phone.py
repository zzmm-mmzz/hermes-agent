...
class Base():
    ...
    def _filter_by_work_type(self, page2, page1, step, flow):
        ...
        page2.locator("span.el-tree-node__label").get_by_text('电费管理-电话号码核实').wait_for(state='visible', timeout=5000)
    ...
    def _signin_work_orders(self, page2, page1, step, flow):
        ...
        try:
            ...
            report(step, "已筛选未处理的电话号码核实工单", flow=flow)
            # remote_log("SELECT","已筛选未处理的电话号码核实工单",batch_id=step)
            # 强制等待
            time.sleep(0.5)
