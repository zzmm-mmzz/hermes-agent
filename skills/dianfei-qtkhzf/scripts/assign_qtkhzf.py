...
class Base():
    ...
    def _select_work_type(self, page3, step, flow):
        """选择工单类型：渠道运营-客户服务-客户其他服务工单"""
        page3.locator('label:text("工单类型")+div input').click()
        # time.sleep(2)

        page3.locator('body>div.el-cascader__dropdown span.el-cascader-node__label').get_by_text("渠道运营").click()
        # time.sleep(1)
        page3.locator('body>div.el-cascader__dropdown div.el-cascader-menu span.el-cascader-node__label').get_by_text('客户服务').click()
        # time.sleep(1)
        page3.locator('body>div.el-cascader__dropdown div.el-cascader-menu span.el-cascader-node__label').get_by_text('客户其他服务工单').click()
        # time.sleep(1)
    ...
    @step_wrapper
    def _fill_work_content(self, page3, cons_no, cons_nm, step, flow):
        """填写工单内容并保存"""
        order_text = """ 【工单内容】
        对编号为【{cons_no}】的用户【{cons_nm}】开展现场走访工作。
        【处理要求】
        1.收集客户诉求并现场处理，不能当场处理的登记上报；
        2.推广属地电话、一户一贴、客户入社群等；
        3.根据工单内容上传相关佐证照片；
        4.请注意现场作业安全。"""
        work_order_content = order_text.format(cons_no=cons_no, cons_nm=cons_nm)
        page3.locator('div[title="工单内容"] textarea').click()
        page3.locator('div[title="工单内容"] textarea').fill(work_order_content)
        # time.sleep(2)
        # page3.locator('div[title="工单名称"] input').fill("客户其他服务工单")
    ...
    def _assign_handler(self, page3, page1, empl, step, flow):
        ...
        # time.sleep(1)
        report(step+1, "完成其他客户走访工单创建", flow=flow)
        remote_log("CREATE", "完成其他客户走访工单创建", batch_id=step+1, workst_no=title, workst_tp='其他客户走访')
        # time.sleep(1)
        print(f"其他客户走访工单已创建{title}")
    ...
    def main(self, **kwargs):
        ...
        """
        mode = get_mode()
        flow_name = '创建其他客户走访工单'
        cons_no = kwargs.get('cons_no', '')
        empl = kwargs.get('empl', '')

        if not cons_no or not empl:
            print("错误：缺少用电户编号、接单人参数")
            return
        ...
        with ExitStack() as stack:
            ...
            self._fill_work_content(page3, cons_no, cons_nm, step=9, flow=flow_name)
