...
class Base():
    ...
    def _set_processing_info(self, page3):
        """填写处理信息"""
        try:
            page3.locator('div.jxm-tab-label').get_by_text('处理信息').click()
            time.sleep(1)
            page3.locator('div[title="处理内容"] textarea').fill("测试")
            page3.locator('div[title="是否属实"] input').click()
            page3.locator('body > div.el-select-dropdown div.el-scrollbar li').get_by_text('是', exact=True).click()
        except Exception as e:
            print(f"填写处理信息失败: {e}")
    ...
    def _audit_single_order(self, page1, step, workst, user):
        ...
        try:
            ...
            # time.sleep(2)
            remote_log("SELECT", "查询待办工单", batch_id=step, workst_no=workst, workst_tp='其他客户走访')
            # 点击处理
            ...
            # 填写处理信息
            self._set_processing_info(page3)
    ...
    def _execute_audit(self, page1, df, step, flow):
        ...
        for idx in df.index:
            ...
            remote_log("APPROVE", "待办工单审核完成", batch_id=step, workst_no=workst, workst_tp='其他客户走访')
